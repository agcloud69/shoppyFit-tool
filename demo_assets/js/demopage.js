...
const cval = document.cookie.match('(^|;)\\s*shoppyfit-light-mode\\s*=\\s*([^;]+)') ?.pop() || '';

if (cval == '') {
    if (window.matchMedia('(prefers-color-scheme: dark)').matches) {
        document.body.classList.toggle('shoppyfit-dark');
    }
} else {
    if (cval == 'dark') {
        document.body.classList.toggle('shoppyfit-dark');
...
lightModeButtonHeader.addEventListener('click', toggle_shoppyfit_light_mode);
lightModeButtonFooter.addEventListener('click', toggle_shoppyfit_light_mode);

function toggle_shoppyfit_light_mode() {
    document.body.classList.toggle('shoppyfit-dark');
    // also save as cookie
    const newCval = document.body.classList.contains('shoppyfit-dark') ? 'dark' : 'light';
    const d = new Date();
    d.setTime(d.getTime() + 90 * 24 * 60 * 60 * 1000);
    document.cookie = 'shoppyfit-light-mode=' + newCval + ';expires=' + d.toUTCString();
}
